        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-8 max-w-2xl">
            <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-4 gap-3">
                    <div>
                        ...
                        <select name="title" id="title"
                            ...
                            <option value="Master" {{ old('title', $customer->title) === 'Master' ? 'selected' : '' }}>Master</option>
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="tier" id="tier" required
                            ...
                            @foreach($tiers as $t)
                                <option value="{{ $t->tier }}" {{ old('tier', $customer->tier) === $t->tier ? 'selected' : '' }}>
                                    {{ $t->tier }}{{ $t->discount_percentage > 0 ? ' ('.$t->discount_percentage.'% off)' : '' }}
                                </option>
                            @endforeach
                            {{-- If customer's current tier is inactive, still show it --}}
                            @if($customer->tier && $tiers->pluck('tier')->doesntContain($customer->tier))
                                <option value="{{ $customer->tier }}" selected>{{ $customer->tier }} (inactive)</option>
                            @endif
                        </select>
                ...
                <div class="flex items-center gap-2 text-sm text-gray-500">
                    Current tier:
                    <span id="tier-badge-preview" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border {{ $customer->tierBadgeClass() }}">
                        {{ $customer->tier ?? 'None' }}
                    </span>
                    <span id="tier-discount-hint" class="text-xs text-green-600 font-medium"></span>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="slmc_registration_number" id="slmc_registration_number"
                        value="{{ old('slmc_registration_number', $customer->slmc_registration_number) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="print_tier_on_receipt" id="print_tier_on_receipt" value="1"
                        ...
                        {{ old('print_tier_on_receipt', $customer->print_tier_on_receipt) ? 'checked' : '' }}>
    ...
    <script>
        ...
        function updateBadge(tier) {
            ...
            if (data) {
                ...
                discHint.textContent = data.discount > 0 ? `${data.discount}% discount at POS` : '';
            } else {
                tierPreview.className = `${base} bg-gray-100 text-gray-500 border-gray-300`;
                tierPreview.style = '';
                tierPreview.textContent = tier || 'None';
